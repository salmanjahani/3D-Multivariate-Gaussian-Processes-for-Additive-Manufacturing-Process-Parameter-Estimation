...
SS316L_Cu=SS316L_Cu[complete.cases(SS316L_Cu[,5]),]
...
# new=data.frame(id=3,Power=rep(seq(min(SS316L_Cu$Power),max(SS316L_Cu$Power),length.out=30),each=30*6),Velocity=rep(rep(seq(min(SS316L_Cu$Velocity),max(SS316L_Cu$Velocity),length.out = 30),each=6),30),Hatch=rep(seq(min(SS316L_Cu$Hatch),max(SS316L_Cu$Hatch),length.out = 6),30*30),Density=NA,Wa=NA)
new=data.frame(id=3,Power=rep(seq(min(SS316L$Power),max(Cu$Power),length.out=10),each=10*5),Velocity=rep(rep(seq(min(Cu$Velocity),max(SS316L$Velocity),length.out = 10),each=5),10),Hatch=rep(seq(min(SS316L_Cu$Hatch),max(SS316L_Cu$Hatch),length.out = 5),10*10),Density=NA,Sa=NA)
...
while(iter<1){
  ...
  data_iter=rbind(data_iter,data.frame(Iteration=iter,Power=predictionpoints[,1],Velocity=predictionpoints[,2],Hatch=predictionpoints[,3],Density_Prediction=as.numeric(ypred2),SE=se))
}
# ############################################ Results - Write to file ######################################################
#write.csv(ae,file="C:/Users/rankouhi/Box/Projects/Machine Learning/Data/AbsoluteError.csv")
#write.xlsx(data_iter,file="C:/Cloud Drives/Box Sync/Projects/Machine Learning/Data/Results/Extended/New/Density.xlsx")
#write.csv(data_iter,file="C:/Users/jahan/Box/Data/Predictions_extended_2.xlsx")

for(i in unique(data_iter$Hatch)){
  data2=data_iter[data_iter$Hatch==i,]
  write.xlsx(data2,file=paste("C:/Cloud Drives/Box Sync/Projects/Machine Learning/Data/Results/Predictions_extended_Density",i,".xlsx"))
  #"C:/Cloud Drives/Box Sync/Projects/Machine Learning/Data/Results/Predictions_extended.csv"
}
